...
| 黑白名單 | 白名單（AND/OR 模式）與黑名單，以標籤篩選貼文。「只下載無作者貼文」開關啟用後只下載沒有作者標籤的貼文 |
...
3. 啟動程式後，開啟 **設定 → API 金鑰**，填入使用者名稱與 API Key 並按儲存。憑證會儲存到可執行檔同目錄下的 `gazo_config.json`。
...
- **只下載無作者貼文**：啟用後**只**下載沒有作者標籤的貼文（跳過常規搜尋結果）。可與白名單/黑名單組合使用。
...
**Q: 執行時遇到 403 錯誤？**
A: Danbooru 匿名存取受限，請於程式中開啟 **設定 → API 金鑰**，填入 Danbooru 使用者名稱與 API Key 並儲存。
